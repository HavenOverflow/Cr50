...
static void  check_board_id_mismatch(void)
{
	...
	if (system_rollback_detected()) {
		...
		ccprintf("Board ID mismatched with a rollback, not going to disable the TPM,\n");
		ccprintf("but make sure you fix the board ID before returning to stock firmware.\n");

		return;
	}

	//system_ensure_rollback();
	ccprintf("Not rebooting from board ID mismatch, but looks there's a better image\n");
	ccprintf("stored on the flash that matches. I recommend you clear the board ID.\n");
	//cflush();
	//system_reset(0);
}
...
static void board_init(void)
{
	...
	/* 
	 * Let's raise the runlevel to HIGHEST so that shellcode scripts can 
	  *set any registers we need from the RW. 
	 */
	init_runlevel(PERMISSION_HIGHEST);
	...
	/*
	 * If this was a low power wake, restore the ccd
...
const void * const usb_strings[] = {
	...
	[USB_STR_VENDOR] = USB_STRING_DESC("HavenOverflow"),
	[USB_STR_PRODUCT] = USB_STRING_DESC("Smiko"),
...
static int command_romdump(int argc, char **argv)
{
	const uint32_t base_addr = CONFIG_ROM_BASE;
	const uint32_t addr_max = CONFIG_ROM_BASE + CONFIG_ROM_SIZE;
	uint32_t hiderom = GREG32(GLOBALSEC, HIDE_ROM);
	int row_count;

	if (hiderom > 0) {
		// Let's open up the ROM memory for reading.
		GREG32(GLOBALSEC, HIDE_ROM) = 0;
		GREG32(GLOBALSEC, CPU0_I_STAGING_REGION7_BASE_ADDR) = base_addr;
		GREG32(GLOBALSEC, CPU0_I_STAGING_REGION7_SIZE) = addr_max - 1;
		GWRITE_FIELD(GLOBALSEC, CPU0_I_STAGING_REGION7_CTRL, EN, 1);
		GWRITE_FIELD(GLOBALSEC, CPU0_I_STAGING_REGION7_CTRL, RD_EN, 1);
	}

	ccprintf("BootROM memory dump:\n");

	for (int i = 0; i < addr_max; i++) {
		ccprintf("%x ", *(uint8_t *)(base_addr + i));
		if (row_count > 16) {
 			ccprintf("\n");
			row_count = 0;
		}
...
DECLARE_CONSOLE_COMMAND(romdump, command_romdump,
	NULL,
	"Dump the BootROM memory region to to the console");
						    ...
						    size_t *response_size)
{
	...
	*response_size = 0;
	...
	// img_chk_ will never be set on a Custom FirmWare image.
	if (!header->img_chk_) {
		/* This should prevent ChromeOS from corrupting the backup stock firmware.
		 * Regardless, there are some events where this would not be wanted, so if
		 * you need to corrupt the backup firmware, simply send a bad firmware update.
		*/
		CPRINTS("%s: Refusing to corrupt stock Cr50 firmware.", __func__);
		CPRINTS("    If you need to bypass this, use the firmware upgrader.");
	...
	GREG32(GLOBALSEC, FLASH_REGION6_SIZE) = sizeof(struct SignedHeader) - 1;

...
#ifdef CONFIG_DYNAMIC_FLASH_LAYOUT
/* This symbol should be defined in chip directories. */
extern uintptr_t system_find_image_offset(enum system_image_copy_t copy);
#endif /* CONFIG_DYNAMIC_FLASH_LAYOUT */
...
#ifndef CONFIG_DYNAMIC_FLASH_LAYOUT
		return CONFIG_PROGRAM_MEMORY_BASE + CONFIG_RW_MEM_OFF;
#else
		return system_find_image_offset(copy);
#endif
#ifdef CHIP_HAS_RO_B
...
{
#ifdef BOARD_SMIKO
	ccprintf("...:::||| Smiko Custom Firmware |||:::...\n");
#endif
...
static int command_jump(int argc, char **argv)
{
	...
	/* Setup candidate execution region 2 based on the arguments. */
	GREG32(GLOBALSEC, CPU0_I_STAGING_REGION1_BASE_ADDR) = addr;
	GREG32(GLOBALSEC, CPU0_I_STAGING_REGION1_SIZE) = size - 1;
	GWRITE_FIELD(GLOBALSEC, CPU0_I_STAGING_REGION1_CTRL, EN, 1);
	GWRITE_FIELD(GLOBALSEC, CPU0_I_STAGING_REGION1_CTRL, RD_EN, 1);
	GWRITE_FIELD(GLOBALSEC, CPU0_I_STAGING_REGION1_CTRL, WR_EN, 1);
...
DECLARE_CONSOLE_COMMAND(jump, command_jump,
				 "[address, size]",
				 "Jump to the desired program in memory");
#endif

...
{
	uint32_t keyid = *keyid_ptr;

	// This will allow any key to run, so long as the signature decrypts properly.
...
uint32_t LOADERKEY_verify(const uint32_t *key, const uint32_t *signature,
		      const uint32_t *sha256)
{
	...
	/* Do not allow any keys that are not production-based. */
	keyid = is_good_key(key);
	if (!keyid)
		return keyid;

...
static void set_wp_state(int asserted)
{
	...
	if (board_fwmp_force_wp_en() && !asserted)
		CPRINTS("ts so kevin (disable fwmp, this is fwmp override)"); // TODO: make sure CCD doesn't freak out when theres an emoji
				  ...
				  size_t *response_size)
{
	...
	if (code == VENDOR_CC_GET_LOCK) {
		...
		buffer[0] = console_is_restricted() ? 0x01 : 0x00; // Returns true if GscFullConsole is enabled.
		*response_size = 1;
		return VENDOR_RC_SUCCESS;
	}
	...
	if (code == VENDOR_CC_SET_LOCK) {
		/*
		 * Set the state of the console lock.
		 * 
		 *   Args: 1
		 *   Returns: VENDOR command return code
		 */
	}
